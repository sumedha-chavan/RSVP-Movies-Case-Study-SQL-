...
SELECT year, COUNT(id) as number_of_movies_released_each_year
FROM movie
GROUP BY year
ORDER BY year;

-- Number of movies released each month.
SELECT MONTH(date_published) AS month_num, COUNT(id) AS number_of_movies_released_each_month
FROM movie
...
-- Type your code below:
SELECT genre, year, COUNT(movie_id) AS movie_count
FROM genre AS genre_name
INNER JOIN movie AS movie_name
ON genre_name.movie_id = movie_name.id
WHERE year = 2019
GROUP BY genre
